...
Everything not in the Drive folder is still in this repository. Nothing was lost: the 42
tables, 16 figures and 56 forecast files are all tracked in git, and the untracked model
...
### The eight left out (in the repo at `results/figures/`)
...
### Deliberately left out (in the repo at `results/tables/`)
...
`main` byte for byte, verified after upload. Figures were copied server-side within Drive;
tables were uploaded from the local repo.

**One open item on Drive:** FIG4 and FIG8 were regenerated by the fixes above, and PNGs of
that size cannot be pushed through the Drive tooling in use. The superseded copies sit in
`Final Output/figures/` renamed `OUTDATED_DO_NOT_USE__…`, and
`Datasets/00_DOCUMENTATION/ACTION_REQUIRED_replace_two_figures.md` has the upload
instruction. The repo copies are already correct — this affects Drive only.

Regenerate everything from scratch with the scripts in `Datasets/10_SCRIPTS/` (41–50).
