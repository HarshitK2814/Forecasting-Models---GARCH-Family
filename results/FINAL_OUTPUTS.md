...
**Drive location:** `Output/FINAL_OUTPUTS/` — contains `figures/` and `tables/` and nothing
else, per Arham's spec of 2026-08-31. This document lives in
`Datasets/00_DOCUMENTATION/README_FINAL_OUTPUTS.md` on Drive so it does not clutter the
outputs folder.
...
`Datasets/00_DOCUMENTATION/ACTION_REQUIRED_replace_two_figures.md` has the four-file upload
